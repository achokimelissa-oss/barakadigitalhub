...
const ImpactPage = ({ goBack, isMobile }) => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "linear-gradient(160deg,#f8fbff 0%,#eef9ff 50%,#f0fdf4 100%)", paddingTop: 48 }}>
      <PageHelmet title="Social Impact | Baraka Digital Hub" description="How Baraka Digital Hub creates sustainable employment for Nairobi youth while delivering world-class AI data services." robots="noindex,follow" canonical="/" />
...
const BarakaDigitalHub = () => {
  ...
  return (
    <div style={{ minHeight: "100vh", background: "transparent", fontFamily: "'DM Sans', sans-serif", overflowX: "hidden" }}>
      <style>{`
      ...
      <nav style={{ position: "fixed", top:0, left:0, right:0, zIndex: 100, transition:"all .3s", background: (scrolled||page!=="home") ? "rgba(255,255,255,.98)" : "transparent", backdropFilter: (scrolled||page!=="home") ? "blur(20px)" : "none", borderBottom: (scrolled||page!=="home") ? "1px solid rgba(0,0,0,.08)" : "none", padding: "0", color: "#0f172a" }}>
        <div className="page-container" style={{ display:"flex", alignItems:"center", justifyContent:"space-between", height: scrolled ? 84 : 88, padding: "14px 24px" }}>
      ...
      {page !== "home" ? (
        <div style={{ position: 'relative', minHeight: '100vh' }}>
          <div aria-hidden style={{ position: 'fixed', inset: 0, pointerEvents: 'none', zIndex: 1 }}>
            <div style={{ position: 'absolute', inset: 0, background: 'none', opacity: 0, pointerEvents: 'none' }} />
          </div>
            ...
            <div>
              ...
              <div style={{ display: "grid", gap: 8 }}>
                {['/', '/services', '/impact', '/blogs'].map((p) => {
                  ...
                  return (
                    <button key={p} onClick={() => navigate(to)} style={{ textAlign: "left", background: "none", border: "none", color: "rgba(255,255,255,0.85)", padding: 0, cursor: "pointer", fontSize: "0.95rem", fontFamily: "inherit" }} onMouseEnter={e=>e.target.style.color="white"} onMouseLeave={e=>e.target.style.color="rgba(255,255,255,0.85)"}>{label}</button>
                  );
                })}
              </div>
            </div>
      ...
      {scrolled && (
        <button onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} aria-label="Scroll to top" className="scroll-top visible">
          ↑
        </button>
      )}
    </div>
